...
**Architecture:** Next.js 15 App Router with React Server Components for the frontend, Supabase for database/auth/storage, GLM API for AI text generation, and Satori for edge-rendered share posters. All deployed on Vercel.

**Tech Stack:** Next.js 15, TypeScript, Tailwind CSS, Supabase (PostgreSQL + Auth), 智谱 GLM API, Satori/Vercel OG, Vercel deployment.
...
│   │   └── api/
│   │       ├── generate/
│   │       │   └── route.ts        # POST: GLM news generation endpoint
│   │       ├── vote/
...
│   │   ├── glm.ts                  # GLM API client + prompt templates
...
### Task 5: GLM API integration + prompt templates

**Files:**
- Create: `src/lib/glm.ts`, `src/app/api/generate/route.ts`
...
If the `zhipuai` npm package is not available, use raw fetch instead (the implementation below supports both).

- [ ] **Step 2: Create GLM client with prompt templates**
...
```bash
git add -A
git commit -m "feat: add GLM API integration with 4 news template prompts"
```
...
```bash
git add -A
git commit -m "feat: add publish page with template selector and GLM generation"
```
...
- Publish flow: template → input → GLM generation → preview → publish
...
| 5 | GLM integration | lib/glm.ts, api/generate |
